...
async function startServer() {
  ...
  // Helper for Proxying
  const proxyRequest = async (req: any, res: any, targetPath: string, method: 'get' | 'post' | 'put' | 'patch' | 'delete' = 'get') => {
    if (USE_DEMO_FALLBACK) {
      return null; // Signals to use local fallback
    }

    if (!FLASK_API_BASE_URL) {
      console.error("FLASK_API_BASE_URL not configured and USE_DEMO_FALLBACK is false");
      res.status(500).json({ ok: false, error: "Servidor backend no configurado", status: 500 });
      return;
    }
    ...
    try {
      ...
      if (res.headersSent) return { _proxied_error: true };

      const status = error.response?.status || 500;
      const errorMessage = error.response?.data?.error || error.response?.data?.message || error.message || "Error en comunicación con el backend";
      
      console.error(`Error proxying to ${targetPath}:`, errorMessage);
  ...
  app.get("/api/advisor/repurchase-suggestions", authenticateToken, async (req, res) => {
    ...
    if (data === null) {
      ...
      if (customer_id) {
        filtered = allSuggestions.filter(s => s.customer_id === (typeof customer_id === 'string' ? customer_id : ''));
      }
  ...
  app.post("/api/advisor/visits/:visitId/check-in", authenticateToken, async (req, res) => {
    ...
    if (data === null) {
      return res.json({ ok: true, status: "in_progress", message: "Check-in demo exitoso" });
    }
    if (data._proxied_error) return;
    res.json(data);
  });
  ...
  app.post("/api/advisor/visits/:visitId/check-out", authenticateToken, async (req, res) => {
    ...
    if (data === null) {
      return res.json({ ok: true, status: "completed", message: "Check-out demo exitoso" });
    }
    if (data._proxied_error) return;
    res.json(data);
  });
  ...
  app.post("/api/advisor/customers/:customerId/location", authenticateToken, async (req, res) => {
    ...
    if (data === null) {
      return res.json({ ok: true, message: "Ubicación actualizada demo exitosamente" });
    }
    if (data._proxied_error) return;
    res.json(data);
  });
  ...
  app.post("/api/advisor/orders", authenticateToken, async (req, res) => {
    ...
    if (data === null) {
      return res.json({ ok: true, order_id: "S-88899", name: "SO/00123" });
    }
    if (data._proxied_error) return;
    res.json(data);
  });
